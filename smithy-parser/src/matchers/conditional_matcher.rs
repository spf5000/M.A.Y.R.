...
fn conditional_matcher<'a, P, A, Cond>(parser: P, condition: Cond) -> impl Parser<'a, A>
where
    P: Parser<'a, A>,
    Cond: Fn(&A) -> bool
{
...
mod tests {
    ...
    fn conditional_matcher_whitespace_matches_test() {
        let matcher = conditional_matcher(any_char_matcher, |c| c.is_whitespace());
        assert_eq!(
    ...
    fn conditional_matcher_alphabetical_matches_test() {
        let matcher = conditional_matcher(any_char_matcher, |c| c.is_alphabetic());
        assert_eq!(
    ...
    fn conditional_matcher_with_vector_matches() {
        let alphabetical_matcher = conditional_matcher(any_char_matcher, |c| c.is_alphabetic());
        let matcher = conditional_matcher(repeat_matcher(alphabetical_matcher), |vector| !vector.is_empty());
        assert_eq!(
